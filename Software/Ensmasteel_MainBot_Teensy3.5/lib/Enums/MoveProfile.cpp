...
void MoveProfiles::setup() {
    ...
    get(brake,true)->set(0,0,11,0,10,0.0005,100,0.001,0.001); //On est pas regardant quant a la position (monnitored by epsilon) mais on regarde la vitesse (monitored by dEpsilon)
    ...
    get(accurate,true)->set(75,0.3,11.25,1,0.001,0.0005,0.01,0.5,0.2);
    get(accurate,false)->set(0.87, 0.75, 0.25,1, 0.008, 0.001, 0.05,1.5,1.0);
    
    get(standard,true)->set(75, 11.25, 7.125,1, 0.10, 0.05, 0.20,1.0,0.7); 
    get(standard,false)->set(0.87, 0.25, 0.25,1, 0.05, 0.01, 0.05,3.2,2.5);

    get(fast,true)->set(52.5, 15, 45,1, 0.15, 0.05, 0.1,2.5,1.2);
    get(fast,false)->set(0.87, 0.25, 0.25,1, 0.05, 0.01, 0.05,10.0,5.0);
    
    get(recallage,true)->set(75,0.3,11.25,1,0.005,0.05,0.01,0.2,0.1);
    get(recallage,false)->set(0.87, 0.25, 0.25,1, 0.008, 0.001, 100,0.1,0.1);
}
