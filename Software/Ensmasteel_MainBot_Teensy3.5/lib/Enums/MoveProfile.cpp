...
void MoveProfiles::setup() {
    ...
    get(brake,false)->set(0,0,600,0,100,0.01,100,0.003,0.003); //On est pas regardant quant a la position (monnitored by epsilon) mais on regarde la vitesse (monitored by dEpsilon)

    get(accurate,true)->set(3000,500,1000,1,0.001,0.0005,0.01,0.5,0.3);
    get(accurate,false)->set(3500, 500, 500,1, 0.008, 0.001, 0.05,1.5,1.0);
    
    get(standard,true)->set(3500, 300, 1000,1, 0.10, 0.05, 0.20,1.0,0.7);
    get(standard,false)->set(3500, 300, 500,1, 0.05, 0.01, 0.05,3.2,2.5);

    get(fast,true)->set(1200, 100, 2500,1, 0.15, 0.05, 0.1,2.0,1.0);
    get(fast,false)->set(1920, 300, 100,1, 0.05, 0.01, 0.01,6.5,3.5);

    get(recallage,true)->set(3000,50,500,0,0.001,0.0005,0.01,0.2,0.1);
    get(recallage,false)->set(500, 0, 500,0, 0.008, 0.001, 100,0.1,0.1);
}
